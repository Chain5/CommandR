package it.emgtech.commandr.util;

public class MessageResponse {

    public static final String TOURNAMENT_NOT_VALID = "Tournament not valid.";
    public static final String PASSWORD_NOT_VALID = "Password not valid.";
    public static final String ALL_MATCHES_NOT_CONCLUDED = "All matches are not concluded.";
    public static final String NO_PLAYER_IN_TOURNAMENT = "No player in tournament.";
    public static final String NOT_ENOUGH_PLAYERS = "Not enough players.";
    public static final String MAX_MATCH_GENERATION_REACHED = "Max match generation counter reached.";
    public static final String NO_PLAYER_FOUND = "No player found.";
    public static final String NICKNAME_ALREADY_IN_USE = "Nickname already in use";
    public static final String NO_SCORE_BOARD_TO_UPDATE = "No score board to update.";
}
